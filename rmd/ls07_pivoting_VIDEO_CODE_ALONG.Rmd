...
WIDE datasets have different observational units in defined columns
LONG datasets have different observational units in defined rows
:::
...
infant_deaths_wide %>% 
  ...
  # new columns created with names "name" and "value"
```
...
-   Dataset of patient records from an Ebola outbreak in Sierra Leone in 2014
...
ebola <- 
  ...
  as_tibble() %>% 
  mutate(year = lubridate::year(date_of_onset)) %>%  # extract year from the date
  select(patient_id = id, district, year_of_onset = year)  # selects and rename columns
```
...
-   You could try an approach like this with rowwise():
