---
title: 'Lesson notes | Advanced pivoting'
output:
  html_document:
...
## autograders
suppressMessages(source(here::here("autograder/ls08_advanced_pivoting_autograder.R")))
```
...
.CHECK_Q_adult_long() # Check your answer by running this function
.HINT_Q_adult_long()
...
In such cases, you'll need to provide the string "\\." to indicate a dot "." because the "." is a special character in R, and sometimes needs to be "escaped" with slashes:
...
To learn more about escaping, you can visit this [site.](https://cran.r-project.org/web/packages/stringr/vignettes/regular-expressions.html#escaping){target="_blank"} 
...
Q_diet_diversity_vietnam_long <-
  diet_diversity_vietnam_wide %>%
  rename(enerc_kcal_w__1 = enerc_kcal_w_1,
...
::: practice
Take the `tb_visits_long` dataset that we manipulated above and pivot it back to a wide format.

```{r, eval = FALSE}
Q_tb_visit_wide <-
  tb_visits_long %>%
  pivot_wider(
    names_from = visit_count,
