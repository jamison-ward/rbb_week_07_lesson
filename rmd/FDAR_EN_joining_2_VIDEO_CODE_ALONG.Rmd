...
Solution: Remove the trailing space after "India" in df2, capitalize the "I"  in "indonesia" in df2, and correctly spell "Philipines" in df2
...
inner_join(tb_notifs_public, child_public_clean) %>% 
  arrange(-tb_child_notifs)

# Uttar Pradesh and Madhya Pradesh have the highest number of pediatric TB cases
```
...
The ISO codes are short, standardized labels for each country, so they are unlikely to be inconsistent between data sets.
:::
