...
cbind(demographic, test_info)  # Simply combine data sets by appending columns
```
...
test_info_disordered <- 
  tribble(~name,     ~test_date,    ~result,
          ...
          "Charlie", "2023-07-15",  "Negative")
test_info_disordered
cbind(demographic, test_info_disordered)  # Produces an invalid dataset
```
...
```{r eval = F}
cbind(demographic, test_info_multiple)  # Error due to mismatch in number of rows in the two data frames
```
...
left_join(x = demographic, y = test_info, by = "name")  # Specifies the column that should be the key when combining data sets
```
...
```{r}
left_join(patient_details, vaccination_records,
...
```{r}
left_join(tb_cases_children, country_continents,
...
```{r}
inner_join(tb_2019_africa, highest_exp)
```

If you do this correctly, there will be just one row returned. Why? The only country in the highest_exp data set located in Africa is "Seychelles"
:::
